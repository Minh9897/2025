...
def visulize_person(img, bboxes, vbboxes):
...
if __name__ == '__main__':
    import glob
    detector = insightface.model_zoo.get_model('./scrfd_person_2.5g.onnx')
    detector.prepare(0, nms_thresh=0.5, input_size=(640, 640))
    img_paths = glob.glob('images/*.jpg')
    for img_path in img_paths:
        img = cv2.imread(img_path)
        bboxes, vbboxes = detect_person(img, detector)
        out_image = visulize_person(img, bboxes, vbboxes)
        filename = img_path.split('/')[-1]
        cv2.imwrite('./outputs/%s'%filename, out_image)
